...
def load_day(client, csv_path: str, ds: str) -> int:
    ...
    """
    # ВАЖНО: event_ts должен попасть в insert_df как настоящий datetime, а не
    # строка. clickhouse-connect сам приводит python-типы к типам колонок
    # ClickHouse (DateTime -> нужен .timestamp()); если прочитать колонку как
    # str (как было раньше, dtype=str "для надёжности с ID"), вставка падает с
    # AttributeError: 'str' object has no attribute 'timestamp'. ID-колонки
    # (event_id, seller_id и т.д.) и так остаются строками при обычном чтении —
    # они никогда не выглядят как числа в нашем генераторе (UUID / "S104").
    df = pd.read_csv(csv_path)
    df["event_ts"] = pd.to_datetime(df["event_ts"])
    day_slice = df[df["event_ts"].dt.strftime("%Y-%m-%d") == ds]
